	...
	public void run() {
		while (true) {
			if (!DBJobQueue.isEmpty()) {
				...
				String Sender_pnum = null;
				try {
					Sender_pnum = (String) JSONMsg_in.get("Sender_pnum");
				} catch (JSONException e) {
					e.printStackTrace();
				}
				String Msg_type = null;
				try {
					Msg_type = (String) JSONMsg_in.get("Msg_type");
				} catch (JSONException e) {
					e.printStackTrace();
				}
				...
				switch (Msg_type) {
					case "Reg_user":
						...
						try {
							User_name = (String) JSONMsg_in.get("User_name");
						} catch (JSONException e) {
							e.printStackTrace();
						}
						String User_pnum = null;
						try {
							User_pnum = (String) JSONMsg_in.get("User_pnum");
						} catch (JSONException e) {
							e.printStackTrace();
						}
						JSONArray Friend_in_JSONArray = null;
						try {
							Friend_in_JSONArray = JSONMsg_in.getJSONArray("Friend_pnum_list");
						} catch (JSONException e) {
							e.printStackTrace();
						}
					...
					case "Send_msg":
						// 1:1 Chat
						String Friend_pnum = null;
						try {
							Friend_pnum = (String) JSONMsg_in.get("Friend_pnum");
						} catch (JSONException e) {
							e.printStackTrace();
						}
						String Text_msg = null;
						try {
							Text_msg = (String) JSONMsg_in.get("Text_msg");
						} catch (JSONException e) {
							e.printStackTrace();
						}
